...
--  ——————————————————— 手动分割线 监测WIFI  ———————————————————
-- Reacting to wifi events 本家网络监测
-- g_wifiWatcher = hs.wifi.watcher.new( function()
--     local homeSSID = "TOTOLINK_2ada44"
--     if hs.wifi.currentNetwork() == homeSSID  then
--         hs.audiodevice.defaultOutputDevice():setVolume(80) -- We just joined our home WiFi network
--     else
--         hs.audiodevice.defaultOutputDevice():setVolume(0)  -- We just departed our home WiFi network
--     end
-- end
-- ):start()
...
--         diskutil erasevolume HFS+ RamDisk `hdiutil attach -nomount ram://$((20*100*1024*2))`
...
-- watchFileTouchSprite 目录 监测 上传触动精灵
-- g_watchFileTouchSprite = hs.pathwatcher.new("/Volumes/WorkDisk/SyncFloder/WatchFile/touchSpriteLua/",
--     function()
--         hs.osascript.applescript([[tell application "Safari"
--             activate
--             delay 0.5
--             open location "http://www.touchsprite.net/script/index"
--         end tell ]])
--     end
-- ):start()

-- ftp上传PHP到服务器
-- g_ftp_watcherPHPPath = hs.pathwatcher.new("/Volumes/WorkDisk/SyncFloder/WatchFile/PHPSevers/",
--     function(files)
--         for _,file in pairs(files) do
--             if (#file > 1) then
--                 local fileName = last_fileName(file);
--                 qslCurlFtpSentToServer("joys", "100100100", file, "ftp://192.168.31.132")
--                 show("sent To remote -->".. fileName)
--             end
--         end
--     end
-- ):start()
...
-- ssh 上传lua脚本到手机
-- g_ssh_watchluaPath= hs.pathwatcher.new("/Volumes/WorkDisk/SyncFloder/WatchFile/LuaZip/", function(files)
--     for _,file in pairs(files) do
--         if (#file > 1) then
--             local fileName = last_fileName(file);
--             -- qslSSHSentToServer(port, host, localFullFile, serverFullFilePath)
--             qslSSHSentToServer("2222", "root@localhost", file, "/var/mobile/Media/TouchSprite/lua/"..fileName)
--             show("lua saved-->"..fileName);
--         end
--     end
-- end):start()

-- ssh 上传Cy脚本到手机
-- g_ssh_watchCyPath= hs.pathwatcher.new("/Volumes/WorkDisk/SyncFloder/WorkFiles/iOSREBook-master/chapter-3/3.4Cycript/",
--     function(files)
--         for _,file in pairs(files) do
--             if (#file > 1) then
--                 local fileName = last_fileName(file);
--                 -- qslSSHSentToServer(port, host, localFullFile, serverFullFilePath)
--                 qslSSHSentToServer("2222", "root@localhost",
--                 file, "/usr/lib/cycript0.9/com/joys/"..fileName)
--                 show("cy saved-->"..fileName);
--             end
--         end
--     end
-- ):start()

-- ssh Shell to cellPhone
-- qsw_watchShell = hs.pathwatcher.new("/Volumes/WorkDisk/SyncFloder/WorkFiles/shell/",
--     function(files)
--         for _,file in pairs(files) do
--             if (#file > 1) then
--                 local fileName = last_fileName(file);
--                 -- qslSSHSentToServer(port, host, localFullFile, serverFullFilePath)
--                 qslSSHSentToServer("2222", "root@localhost",
--                 file, "/var/root/"..fileName)
--                 show("sh saved-->"..fileName);
--             end
--         end
--     end
-- ):start()

-- local qsFtp = QSFtp('pi', 'joyspace', 'ftp://192.168.31.188/')

-- ftp上传 SyncFloder 到 pi 树莓派
-- g_ftp_watcherSyncFloderPath = false
-- watchFileTouchSprite 目录 监测 上传触动精灵

-- // *************** 监测
-- ftp上传 SyncFloder 到 pi 树莓派
-- g_ftp_watcherSyncFloderPath = hs.pathwatcher.new("/Volumes/WorkDisk/SyncFloder/",
--     function(files, flagTables)
--         for k,file in pairs(files) do
--             if (#file > 1) then
--                 local fileName = last_fileName(file);
--                 --print("flagTables :"..flagTables[k].itemIsFile)
--                 -- .DS_Store
--                 -- qslCurlFtpSentToServer("joys", "100100100", file, "ftp://192.168.31.132")
--                 -- show("sent To remote -->".. fileName)
--             end
--         end
-- -- var =table: 0x604000a60880
-- -- itemChangeOwner = true
-- -- itemRenamed = true
-- -- itemIsFile = true
...
-- ------ 本机脚本监测
-- function reloadConfig(files) for _,file in pairs(files) do if file:sub(-4) == ".lua" then hs.reload(); break; end end end
-- g_watcherReloadConfig = hs.pathwatcher.new(os.getenv("HOME") .. "/.hammerspoon/", reloadConfig):start()
